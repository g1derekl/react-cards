...
var isPointCard = function(card, player, gameState, cards) { // Determine if the card is a Queen of spades or a heart.
  if (card.suit == 'Heart' || _.isEqual(card, {value: 'Q', suit: 'Spade'})) {
    return doesPlayerOnlyHavePointCards(card, player, gameState, cards);
  }
  return isPlayerLeadingNonPointCard(card, player, gameState, cards);
};

var doesPlayerOnlyHavePointCards = function(card, player, gameState, cards) { // Determine if player only has point cards (Queen of spades or hearts).
...
var hasHeartsBeenBroken = function(card, player, gameState, cards) { // Determine if hearts (or the Queen of spades) has been broken.
  if (_.find(cards, {belongsTo: 'discard', suit: 'Heart'}) || _.find(cards, {belongsTo: 'discard', value: 'Q', suit: 'Spade'})) {
    return true;
  }
  return false;
};
...
module.exports = function(data) { // Determine if a move is legal.
  return isPointCard({value: data.card.value, suit: data.card.suit}, data.player, data.gameState, data.cards); // See flow chart for detailed steps.
};