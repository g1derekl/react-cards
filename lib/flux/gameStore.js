...
module.exports = Fluxxor.createStore({ // Game logic goes here.
  ...
  onPlayCard: function(payload) {
    this.waitFor(['CardStore'], function(cardStore) {
      this._playCard(payload.card, cardStore.getState().cards, function(newHand, gameOver){
        this.emit('change');
  ...
  _endOfHand: function() {
    ...
    var gameOver = _.find(this.pointsTotal, function(score) {
      return score >= 100;
    });
